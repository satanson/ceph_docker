...
bootstrap_mysqld(){
  ...
  #local extraFlags="--name ${node} --hostname ${node} --net static_net0 --ip ${ip} -v  ${PWD}/rbd_volume0_mnt:/root/mysqld_data"
...
flush privileges;
create user 'root'@'192.168.172.0/255.255.172.0';
# drop user 'root'@'192.168.172.0/255.255.172.0';
create user 'foobar'@'192.168.172.0/255.255.172.0';
# drop user 'foobar'@'192.168.172.0/255.255.172.0';
set password for 'root'@'192.168.172.0/255.255.172.0' = '123456';
set password for 'foobar'@'192.168.172.0/255.255.172.0' = '123456';
grant all on *.* to 'root'@'192.168.172.0/255.255.172.0';
# revoke insert, update on test.* from 'root'@'192.168.172.0/255.255.172.0';
grant all on *.* to 'foobar'@'192.168.172.0/255.255.172.0';
# revoke all on test.* from 'root'@'192.168.172.0/255.255.172.0';
flush privileges;
